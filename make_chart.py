...
# status palette: red=높음(critical), amber=검토필요(warning), green=양호(good)
data = [
    ("양호 확인 요소", 3, "#2E7D32"),
    ("검토 필요", 9, "#B45309"),
    ("위험 높음", 5, "#C00000"),
...
ax.invert_yaxis()  # 위험 높음이 맨 위로 오도록 순서 반전
...
ax.set_xticks([])
ax.set_title('계약조항 검토 결과 분포 (총 17개 항목)', fontsize=14, fontweight='bold', color='#222222', loc='left', pad=12)

plt.tight_layout()
plt.savefig('chart_risk.png', transparent=True)
print('saved')
